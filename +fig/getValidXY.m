function [x, y, xx] = getValidXY(subjInfo, d, dp)
    ...
    yTF = false(size(x));
    yTF(1 : 3 : end - 2) = all(isnan(dp.(d.EventValidSrc).ValidS), 2); % If it is NaN in all channels, make y true
    yTF(2 : 3 : end - 1) = all(isnan(dp.(d.EventValidSrc).ValidS), 2);
    yTF(1 : 3 : end - 2) = all(dp.(d.EventValidSrc).ValidS == 0, 2); % If there is 0 seconds of valid signal, make y true
    yTF(2 : 3 : end - 1) = all(dp.(d.EventValidSrc).ValidS == 0, 2);
    y = zeros(size(yTF));
